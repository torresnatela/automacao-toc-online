      ...
      <form action={formAction} style={{ display: "grid", gap: 12, maxWidth: 520 }}>
        {isAdmin && (
          <label style={labelStyle}>
            <span>Equipe</span>
            <select name="teamId" defaultValue={initial?.team_id ?? defaultTeamId ?? ""} required>
            ...
            {fe.teamId && <span style={errStyle}>{fe.teamId}</span>}
          </label>
        )}

        <label style={labelStyle}>
          <span>NISS</span>
          <input name="niss" inputMode="numeric" defaultValue={initial ? String(initial.niss) : ""} required />
          {fe.niss && <span style={errStyle}>{fe.niss}</span>}
        </label>
        ...
        <label style={labelStyle}>
          ...
          <select name="type" defaultValue={initial?.type ?? "employer"}>
          ...
          {fe.type && <span style={errStyle}>{fe.type}</span>}
        </label>
        ...
        <label style={labelStyle}>
          ...
          {fe.email && <span style={errStyle}>{fe.email}</span>}
        </label>
        ...
        <label style={labelStyle}>
          ...
          <input name="postalCode" placeholder="1049-013" defaultValue={initial?.postal_code ?? ""} />
          {fe.postalCode && <span style={errStyle}>{fe.postalCode}</span>}
        </label>
        ...
        <label style={labelStyle}>
          <span>País</span>
          <input name="country" maxLength={2} defaultValue={initial?.country ?? "PT"} />
          {fe.country && <span style={errStyle}>{fe.country}</span>}
        </label>
