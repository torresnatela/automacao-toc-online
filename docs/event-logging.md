...
| `rpa.*`         | passos de crawler/Playwright (`rpa.navigate`, `rpa.extract`); Módulo 1: `rpa.at.session`, `rpa.at.iva_declaration`, `rpa.at.payment_document`, e `rpa.toconline.direct_access` (só rota A — Acesso Direto do TOConline; filho de `rpa.at.session`, com os marcos do adaptador no seu log) | herda do trace        |
