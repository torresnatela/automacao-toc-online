-- Módulo 1 — `portal_paused` chega à listagem: a view passa a dizer se o
-- último job está ADIADO.
--
-- Sem esta coluna o estado era inalcançável na interface. Um adiamento devolve
-- o job à fila (`status = 'pending'`, sem gastar tentativa) e escreve
-- `last_error = {"deferred": true}`; a view achatava tudo o que é `pending` em
-- "Na fila", e o operador via 182 empresas eternamente na fila sem saber que o
-- que as segura é uma pausa do Portal das Finanças que se resolve sozinha em 15
-- minutos. `job_deferred` é o único sinal que distingue as duas esperas.
--
-- Correção à nota da migration 20260904015903 (que não se edita, por já estar
-- aplicada): o bucket `documents` é criado **apenas** por ela — o
-- `supabase/config.toml` não o declara, nem local nem em produção. O `on
-- conflict` de lá continua a ser a idempotência de um `db reset`, não uma
-- reconciliação com o config.toml.
--
